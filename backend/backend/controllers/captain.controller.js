...
export const loginCaptain = async function (req, res, next) {
    ...
    try {
        ...
        const token = await captain.generateAuthToken();
        captain.token = token; // logout another captain
        await captain.save();
        res.status(200).cookie("token", token).json({captain, token});
    } catch (error) {
...
export const updateCaptain = async function (req, res, next){
    ...
    try {
        ...
        const updatedCaptain = await captainService.update({email, fullName, captainId: _id, password});
...
export const getCaptain = async function (req, res, next) {
    return res.status(200).json({message: "Captain details fetched successfully", captain: req.captain});
}
